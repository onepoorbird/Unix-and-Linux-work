...
void startsys()
{
    ...
    if ((file = fopen(FILENAME, "r")) != NULL) {
        ...
        if (memcmp(buffer, "10101010", 8) == 0) {
            ...
            printf("zfilesys file read successfully\n");
        } else {
        ...
        printf("zfilesys not find, create file system\n");
        my_format();
        memcpy(buffer, myvhard, SIZE);
    }
...
void my_mkdir(char* dirname)
{
    ...
    fcbtmp->free = 1;
    do_write(currfd, (char*)fcbtmp, sizeof(fcb), 4);
    ...
    fcbtmp->first = block_num;
    fcbtmp->length = 2 * sizeof(fcb);
    do_write(fd, (char*)fcbtmp, sizeof(fcb), 4);
    ...
    do_write(fd, (char*)fcbtmp2, sizeof(fcb), 4);
    ...
    openfilelist[currfd].count = 0;
    do_write(currfd, (char*)fcbptr, sizeof(fcb), 4);
    openfilelist[currfd].fcbstate = 1;
}
...
int my_close(int fd)
{
    ...
    if (openfilelist[fd].fcbstate == 1) {
        ...
        do_write(father_fd, (char*)fcbptr, sizeof(fcb), 4);
    }
    // 释放打开文件表
...
int my_write(int fd)
{
    ...
    while (1) {
        ...
        // 2. 覆盖写
        // 3. 追加写
        printf("1:Truncation 2:Overwrite 3:Addition\n");
        scanf("%d", &wstyle);
    ...
    char text[MAX_TEXT_SIZE];  // 这里修改，不初始化为'\0'，避免后续strlen等操作受影响
    char texttmp[MAX_TEXT_SIZE];
    printf("please input data, line feed + $$ to end file\n");
    getchar();
    int textLen = 0;  // 新增变量记录已输入文本长度
    while (gets(texttmp)) {
        if (strcmp(texttmp, "$$") == 0) {
            break;
        }
        int tmpLen = strlen(texttmp);
        if (textLen + tmpLen + 1 > MAX_TEXT_SIZE) {  // 新增判断避免缓冲区溢出
            printf("Input text exceeds buffer size. Please try again.\n");
            return -1;
        }
        texttmp[tmpLen] = '\n';  // 确保添加换行符正确
        strcpy(text + textLen, texttmp);  // 改为使用strcpy避免潜在的未初始化内存问题
        textLen += tmpLen + 1;
    }
    if(wstyle!=2)text[textLen] = '\0';  // 若不是覆盖写,则手动添加字符串结束符
    if(wstyle!=2)do_write(fd, text, textLen+1, wstyle);  // 传递正确的长度
    else do_write(fd, text, textLen-1, wstyle); //不传递换行符
...
int do_write(int fd, char* text, int len, char wstyle)
{
    ...
    if (wstyle == 1) {
        ...
        // 覆盖写，如果是一般文件，则需要先删除末尾 \0，即将指针移到末位减2个字节处
        openfilelist[fd].count = 0;
    }else if (wstyle == 3) {
    ...
    // 删除多余的磁盘块
    if (wstyle == 1 || (wstyle == 4 && openfilelist[fd].attribute == 0)) {
